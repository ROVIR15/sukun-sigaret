...
class bk_Twitter extends WP_Widget {
	...
	function bk_Twitter() {
		...
		/* Create the widget. */
		$this->WP_Widget( 'bk-twitter', __('*BK: Widget Twitter', 'bkninja'), $widget_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		$this->consumer_key = isset( $instance['consumer_key'] ) ? esc_attr($instance['consumer_key']) : '';
		$this->consumer_secret = isset( $instance['consumer_secret'] ) ? esc_attr($instance['consumer_secret']) : '';
		$this->access_token = isset( $instance['access_token'] ) ? esc_attr($instance['access_token']) : '';
		$this->access_token_secret = isset( $instance['access_token_secret'] ) ? esc_attr($instance['access_token_secret']) : '';
		...
		if ( $title )
			echo $before_title .'<i class="fa fa-twitter"></i>'. esc_attr($title). $after_title;
	...
	function form( $instance ) {
		...
		<input  type="text" id="<?php echo $this->get_field_id( 'show_count' ); ?>" name="<?php echo $this->get_field_name( 'show_count' ); ?>" value="<?php echo esc_attr($instance['show_count']); ?>" size="3" /><?php _e(' tweets', 'bkninja'); ?>
		</p>
	...
	function messages($username = '', $num = 1, $slider = false, $update = true, $linked  = '#') {
		...
		if ($slider) echo '<div id="'.$this->uid.'"><ul class="twitter-list">';
