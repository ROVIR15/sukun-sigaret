    ...
    if (is_user_logged_in()) {
        ...
        $comments_args = array(
            ...
            'fields' => apply_filters( 'comment_form_default_fields', array(
                'author' => '<p class="comment-form-author"><input id="author" name="author" type="text" placeholder="'.__('Name*', 'bkninja').'..." size="30" ' .  $aria_req . ' /></p>',
                'email' => '<p class="comment-form-email"><input id="email" name="email" size="30" type="text" placeholder="'.__('Email*', 'bkninja').'..." '. $aria_req .' /></p>' ) ),
            'id_submit' => 'comment-submit',
            'label_submit' => __('Send', 'bkninja'),
            // redefine your own textarea (the comment body)
            'comment_field' => '<p class="comment-form-comment"><textarea id="comment" name="comment" cols="45" rows="8" aria-required="true" placeholder="' . _x( 'Message', 'noun', 'bkninja' ) . '"></textarea></p>');
    } else {
        $comments_args = array(
        // change the title of send button
        'title_reply'=> __('Post a new comment', 'bkninja'),
        ...
        'fields' => apply_filters( 'comment_form_default_fields', array(
                'author' => '<p class="comment-form-author"><input id="author" name="author" type="text" placeholder="'.__('Name*', 'bkninja').'..." size="30" ' .  $aria_req . ' /></p><!--',
                'email' => '--><p class="comment-form-email"><input id="email" name="email" size="30" type="text" placeholder="'.__('Email*', 'bkninja').'..." '. $aria_req .' /></p><!--',
                'url' => '--><p class="comment-form-url"><input id="url" name="url" size="30" placeholder="'.__('Website', 'bkninja').'..." type="text"></p>') ),
        'id_submit' => 'comment-submit',
        'label_submit' => __('Send', 'bkninja'),
        // redefine your own textarea (the comment body)
        'comment_field' => '<p class="comment-form-comment"><textarea id="comment" name="comment" cols="45" rows="8" aria-required="true" placeholder="' . _x( 'Message', 'noun', 'bkninja' ) . '"></textarea></p>');
